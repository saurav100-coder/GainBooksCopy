...
//    $('#loadingmessage').show();
//    $('#Msg').hide();
//    $.post('/CRM/ajaxEnquiryData', { id: key, start: 1, pSize: a, direction: "F", search: b }, function (data) {
//        loadData(data);
//    })
//}
...
function GetEmployeeData(pageNumber, start, PSize, dir) {
    $('#loading').show();
    $('#loadingmessage').show();
    $.post('/CRM/ajaxEnquiryData', { id: pageNumber, start: start, pSize: PSize, direction: dir }, function (data) {
...
function loadData(data) {
    ...
    $.each(data.data, function (index, item) {
        var m =  index+1;
        var tr = $("<tr id='" + item.Enquiry_Key + "'></tr>");
        tr.html(("<td style='width:10%;'>"+ m + "</td>")
             + ("<td style='width:10%;'>" + item.VisitorName + "</td>")
              + ("<td style='width:10%;'>" + item.ContactNumber + "</td>")
            + ("<td style='width:10%;'>" + item.CurrentOrg + "</td>")
             + ("<td style='width:10%;'>" + item.Email + "</td>")
              + ("<td style='width:10%;'>" + item.TextBusinessType + "</td>")
               + ("<td style='width:10%;'>" + item.TextEnqDate + "</td>")
                  + ("<td style='width:10%;'>" + item.EnqType + "</td>")
        + ("<td style='width:20%;'>" + item.EnqMessage + "</td>")           
        );
...
$(document).ready(function () {
    ...
    $('#example thead tr').on('click', 'th', function () {
        ...
        if (ele != undefined) {
            ...
            $('#loadingmessage').show();
            $('#Msg').hide();
            $.post('/CRM/ajaxEnquiryData', { id: 1, start: 1, pSize: a, direction: "F", search: search, order: o }, function (data) {
                loadData(data);
            })
        }
    });
