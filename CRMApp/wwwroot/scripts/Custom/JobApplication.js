...
function GetEmployeeData(pageNumber, start, PSize, dir) {
    $('#loading').show();
    $('#loadingmessage').show();
    $.post('/CRM/ajaxJobApplicationData', { id: pageNumber, start: start, pSize: PSize, direction: dir }, function (data) {
...
function loadData(data) {
    ...
    $.each(data.data, function (index, item) {
        var m =  index+1;
        var tr = $("<tr id='" + item.Applicant_Key + "'></tr>");
        tr.html(("<td style='width:68px;'>"+ m + "</td>")
            + ("<td style='width:86px'>" + item.Name + "</td>")
             + ("<td>" + item.MobNo + "</td>")
           + ("<td>" + item.Email + "</td>")
            + ("<td>" + item.TextPosition + "</td>")
             + ("<td>" + item.FrmtTimestamp + "</td>")
              + ("<td>" + item.TextStatus + "</td>")
                 + ("<td style='width:10%;'><a href='http://" + item.LinkURL + "' target='_blank' download>" + item.FileName + "</a></td>")
       );
...
$(document).ready(function () {
    ...
    $('#example thead tr').on('click', 'th', function () {
        ...
        if (ele != undefined) {
            ...
            $('#loadingmessage').show();
            $('#Msg').hide();
            $.post('/CRM/ajaxJobApplicationData', { id: 1, start: 1, pSize: a, direction: "F", search: search, order: o }, function (data) {
                loadData(data);
            })
        }
    });
    ...
    build: function ($trigger) {
        var options = {
            callback: function (key, options) {
                ...
                switch (key) {
                    case "edit":
                        location.href = '/CRM/EditJobApplication/' + Rowid;
                        break;
                }
            },
            items: {
