...
//from date will be 30 days back of current date 
function getFromDate() {
    var now = new Date();
    var crtDate = new Date(now.getTime() - (30 * 24 * 60 * 60 * 1000));
    var month = (crtDate.getMonth() + 1) < 10 ? '0' + (crtDate.getMonth() + 1) : (crtDate.getMonth() + 1);
    var day = crtDate.getDate() < 10 ? '0' + crtDate.getDate() : crtDate.getDate();
    var date = crtDate.getFullYear() + "-" + month + "-" + day;
    return date
...
$(document).ready(function () {
    ...
    $("#billdatepickerFrom").val(getFromDate());
    $("#billdatepickerTo").val(getToDate());
...
function GetEmployeeData(pageNumber, start, PSize) {
    ...
    $.ajax({
        type: "POST",
        url: "/Invoices/AjaxGetSaleOrdersDataForGrid",
        data: { start: start, pSize: PSize, search: search },
        success: function (dataStr) {
            if (dataStr.statusCode == 500) {
                window.location.href = "/Home/Error";
            }
...
function loadData(data) {
    ...
    $.each(data.data, function (index, item) {
        ...
        tr.html(("<th class='icon' style='width:4%' ><input type='checkbox' onclick='chkCheckUncheck(this)' class='checkboxall' id='chk-" + item.p_billheader + "' value='" + item.p_billheader + "' style='cursor:pointer' /><a  class ='pull-left-container' onClick='LoadItems(" + item.billheader_key + ", " + item.p_billheader + ")' style='cursor: pointer; padding:5px;'> <i class='careticon fa fa-angle-right' style='color:#105faf; font-size:25px;  margin-top: 10px;padding-bottom: 20px; padding-right:0px;  padding-left:0px; font-weight: 600; z-index:1;'></i></a></th> ")
            + ('<th class="sno basicTr" style="width: 5%; padding-left: 8px;">' + m + '</th>')
            + ('<th class="SaleDate basicTr" style=" width: 7%;">' + item.frmtbilldate + '</th>')
            //+ ('<th class="invoiceid billseries basicTr" style="width:7%;">' + item.billseries + '</th>')
            + ('<th class="invoiceid billno basicTr" style="width:7%; padding-left:20px;">' + item.billno + '</th>')
            + ('<th class="BrandName customer basicTr clamptr" style="width:16%;">' + item.txtcustomer + '</th>')
            + ('<th class="BrandName salesman basicTr" style="width:10%;">' + item.txtsalesman + '</th>')
            + ('<th class="BrandName Delivery basicTr" style="width:10%;">' + item.deliverymode + '</th>')
            ...
            + ('<th class="total basicTr" style="width:9%;">' + item.netamount + '</th>')
            + ('<th class="invoiceid orderid basicTr" style="width:9%;">' + item.orderid + '</th>')
            //+ ('<th class="invoiceid Timestamp basicTr" style="width:10%; padding-left:5px;">' + item.frmtmtimestamp + '</th>')
            + ('<th class="invoiceid Remarks basicTr" style="width:13%;">' + item.remarks + '</th>')
    ...
    if (childRows.length > 0) {
        ...
        $.post('/Invoices/AjaxGetBillItemsDataForPBillheader', { p_billheader: p_billheader }, function (dataStr) {
            ...
            if (orderItems.recordsTotal > 0) {
                var html = (' <tr class="child ChildRow-' + billheader_key + '" style="border-spacing: 5em;border-collapse: collapse; text-align: center;  color: #404040; background-color: #f3f8fa; width: 100%; display:inline-block; padding:2px; ">')
                ...
                + ('<th class="invoiceid" style="width:12%; font-weight:bold !important;">Packing</th>')
                    //+ ('<th class="invoiceid" style="width:12%; font-weight:bold !important;">Batch No.</th>')
                    /*+ ('<th class="invoiceid" style="width:12%; font-weight:bold !important;">Lotorexpry</th>')*/
                    + ('<th class="invoiceid" style="width:12%; font-weight:bold !important;">Expiry Date</th>')
                ...
                /*+ ('<th colspan="6" class="PackingCode" style="width:10%; font-weight:bold !important;">Total</th></tr>');*/
                    + ('<th  class="PackingCode" style="width:10%; font-weight:bold !important;">Total</th></tr>');

                $.each(orderItems.data, function (index, item) {
                    var tr = (' <tr class="child ChildRow-' + billheader_key + '" id="' + item.billitems_key + '" class="" style="text-align: center;  color: #105faf; background-color: white; width: 100%; display:block;">')
                        + ('<th class="blank" style="width: 50px; padding-left: 15px; font-weight:bold !important; cursor:default; color:transparent;">black</th>')
                        + ('<th class="SNo" style="width: 13%; padding-left: 15px !important;">' + item.itemsno + '</th>')
                        + ('<th colspan="2" class="SaleDate" style="width: 30%; font-size:13px; font-weight:600">' + item.prodname + '</th>')
                    ...
                    /*+ ('<th class="invoiceid" style="width:12%; padding-left: 5px !important;">' + item.lotorexpry + '</th>')*/
                       /* + ('<th class="invoiceid" style="width:12%;">' + item.batchno + '</th>')*/
                        + ('<th class="invoiceid" style="width:12%; padding-left: 5px !important;">' + item.txtexpiry + '</th>')
                        + ('<th class="BrandName" style="width:10%;padding-left: 15px !important;">' + item.itemqty + '</th>')
                        + ('<th class="PackingCode" style="width:10%;">' + item.sellingprice + '</th>')
                    /*+ ('<th colspan="6" class="PackingCode" style="width:10%;">' + item.itemamount + '</th> </tr>');*/
                        + ('<th  class="PackingCode" style="width:10%;">' + item.itemamount + '</th> </tr>');
                    html += tr;
                })
