...
function GetEmployeeData(pageNumber, start, PSize, dir) {
    $('#loading').show();
    $('#loadingmessage').show();
    $.post('/CRM/ajaxTodaysTasksComm', { id: pageNumber, start: start, pSize: PSize, direction: dir }, function (data) {
...
$(document).ready(function () {
    ...
    $('#example thead tr').on('click', 'th', function () {
        ...
        if (ele != undefined) {
            ...
            $('#loadingmessage').show();
            $('#Msg').hide();
            $.post('/CRM/ajaxTodaysTasksComm', { id: 1, start: 1, pSize: a, direction: "F", search: search, order: o }, function (data) {
                loadData(data);
            })
        }
    });
