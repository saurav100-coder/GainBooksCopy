...
function GetEmployeeData(pageNumber, start, PSize, dir) {
    $('#loading').show();
    $('#loadingmessage').show();
    $.post('/CRM/ajaxTeamTasksComm', { id: pageNumber, start: start, pSize: PSize, direction: dir }, function (data) {
...
function loadData(data) {
    ...
    $.each(data.data, function (index, item) {
        ...
        tr.html(("<td style='width:5%;'>" + m + "</td>")
             + ("<td style='width:10%;'>" + item.P_CRMTasks + "</td>")
              + ("<td style='width:20%;'>" + item.FrmtCreationDate + "</td>")
            + ("<td style='width:20%;'>" + item.TaskTitle + "</td>")
             + ("<td style='width:20%;'>" + item.Commtext + "</td>")
        + ("<td style='width:20%;'>" + item.TaskDescription + "</td>")
       //+ ("<td style='width:10%;'>" + item.Location + "</td>")
      + ("<td style='width:20%;'>" + item.TextLogincode + "</td>")
        );
...
$(document).ready(function () {
    ...
    $('#example thead tr').on('click', 'th', function () {
        ...
        if (ele != undefined) {
            ...
            $('#loadingmessage').show();
            $('#Msg').hide();
            $.post('/CRM/ajaxTeamTasksComm', { id: 1, start: 1, pSize: a, direction: "F", search: search, order: o }, function (data) {
                loadData(data);
            })
        }
    });
